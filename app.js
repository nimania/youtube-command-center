...
const state = {
  ...
  growthGoal:"revenue",manualIdeas:[],pipeline:[],experiments:[]
};
...
function renderAll(){renderCoach();renderStudio();renderKPIs();renderTrend();renderInsights();renderTopVideos();renderFormats();renderVelocity();renderHeatmap();renderVideoTable();renderSeries();renderAudience();renderRevenue();renderAdvisor();renderComments();renderReport();}
...
function loadCoachState(){try{const saved=JSON.parse(localStorage.getItem("nimaYoutubeCoach")||"{}");if(saved.goals)state.coachGoals={...state.coachGoals,...saved.goals};if(saved.checks)state.coachChecks=saved.checks;if(Array.isArray(saved.extraTasks))state.coachExtraTasks=saved.extraTasks;if(saved.scope)state.coachScope={...state.coachScope,...saved.scope};if(saved.growthGoal)state.growthGoal=saved.growthGoal;if(Array.isArray(saved.manualIdeas))state.manualIdeas=saved.manualIdeas;if(Array.isArray(saved.pipeline))state.pipeline=saved.pipeline;if(Array.isArray(saved.experiments))state.experiments=saved.experiments}catch(e){console.warn("Coach state unavailable",e)}}
function saveCoachState(){try{localStorage.setItem("nimaYoutubeCoach",JSON.stringify({goals:state.coachGoals,checks:state.coachChecks,extraTasks:state.coachExtraTasks.slice(-20),scope:state.coachScope,growthGoal:state.growthGoal,manualIdeas:state.manualIdeas.slice(-50),pipeline:state.pipeline.slice(-100),experiments:state.experiments.slice(-50)}))}catch(e){console.warn("Coach state unavailable",e)}}
...
const growthGoals={revenue:{label:"درآمد",hint:"اولویت با RPM، تماشای عمیق و موضوع‌های درآمدساز"},growth:{label:"رشد مخاطب",hint:"اولویت با جذب مشترک و دسترسی سریع"},watch:{label:"زمان تماشا",hint:"اولویت با ماندگاری و ویدئوهای بلند"},consistency:{label:"انتشار منظم",hint:"اولویت با ایده‌های سریع و قابل‌تکرار"}};
const pipelineStages=[{key:"idea",label:"ایده"},{key:"research",label:"تحقیق"},{key:"script",label:"سناریو"},{key:"record",label:"ضبط"},{key:"edit",label:"تدوین"},{key:"ready",label:"آماده"},{key:"scheduled",label:"زمان‌بندی"}];
function uid(prefix="item"){return`${prefix}-${Date.now()}-${Math.random().toString(36).slice(2,7)}`}
function studioIdeas(){
  const videos=coachAnalysisVideos(),recs=buildRecommendations(videos),topics=topicSignals(videos),avgViews=Math.max(1000,avg(videos,"views")),avgRpm=Math.max(.1,median(videos.map(videoRPM))),goal=state.growthGoal,goalBoost={revenue:8,growth:6,watch:7,consistency:5}[goal]||0;
  const generated=recs.slice(0,5).map((r,i)=>{const format=/(شورت|کوتاه)/.test(`${r.title} ${r.body}`)?"short":"long",base=Math.round(r.score-(i*2)),fit=goal==="revenue"?(format==="long"?goalBoost:-2):goal==="growth"?(format==="short"?goalBoost:2):goal==="watch"?(format==="long"?goalBoost:0):format==="short"?goalBoost:3,views=Math.round(avgViews*(.7+(base/100)*.65)),revenue=views/1000*avgRpm*(format==="short"?.18:1);return{id:`rec-${i}-${r.title.slice(0,18)}`,title:r.action.replace(/^اقدام:\s*/,""),note:r.why,format,score:Math.min(99,Math.max(40,base+fit)),effort:format==="short"?"کم":"متوسط",views,revenue,source:"مربی"}});
  const topicIdeas=topics.slice(0,2).map((t,i)=>({id:`topic-${t.word}`,title:`یک زاویهٔ تازه دربارهٔ «${t.word}»`,note:"موضوعی که در محتوای موفق کانال تکرار شده است",format:i?"short":"long",score:Math.min(92,74+goalBoost-i*3),effort:i?"کم":"متوسط",views:Math.round(avgViews*(1-i*.15)),revenue:avgViews/1000*avgRpm*(i?.2:1),source:"سیگنال موضوعی"}));
  const manual=state.manualIdeas.map((x,i)=>({...x,score:x.score||Math.max(55,76-i),views:x.views||Math.round(avgViews*.65),revenue:x.revenue||avgViews*.65/1000*avgRpm*(x.format==="short"?.18:1),effort:x.effort||"نامشخص",source:"ایدهٔ شما"}));
  return[...manual,...generated,...topicIdeas].sort((a,b)=>b.score-a.score).slice(0,12)
}
function addIdeaToPipeline(idea){if(state.pipeline.some(x=>x.sourceId===idea.id&&x.stage!=="published")){showToast("این ایده از قبل در خط تولید است");return}state.pipeline.unshift({id:uid("content"),sourceId:idea.id,title:idea.title,format:idea.format,stage:"idea",createdAt:new Date().toISOString(),score:idea.score});saveCoachState();renderStudio();showToast("ایده وارد خط تولید شد")}
function renderIdeaQueue(){const root=document.getElementById("ideaQueue");if(!root)return;const ideas=studioIdeas();root.innerHTML=ideas.map((idea,i)=>`<article class="idea-card ${i===0?"top":""}" data-idea-id="${esc(idea.id)}"><div class="idea-score"><strong>${n(idea.score)}</strong><span>امتیاز</span></div><div class="idea-copy"><div><span class="idea-source">${esc(idea.source)}</span><span class="type-badge ${idea.format}">${idea.format==="short"?"شورتز":"بلند"}</span></div><h3>${esc(idea.title)}</h3><p>${esc(idea.note||"")}</p><div class="idea-forecast"><span>بازدید محتمل ${n(idea.views,"compact")}</span><span>درآمد محتمل ${formatDollar(idea.revenue)}</span><span>تلاش ${esc(idea.effort)}</span></div></div><button class="idea-add" data-idea-action="pipeline">ورود به تولید</button></article>`).join("")||`<div class="comment-empty">پس از اتصال کانال، صف ایده ساخته می‌شود.</div>`}
function renderPipeline(){const root=document.getElementById("contentPipeline");if(!root)return;const active=state.pipeline.filter(x=>x.stage!=="published");document.getElementById("pipelineCount").textContent=`${n(active.length)} محتوای فعال`;root.innerHTML=pipelineStages.map((stage,stageIndex)=>{const items=active.filter(x=>x.stage===stage.key);return`<section class="pipeline-column"><header><strong>${stage.label}</strong><span>${n(items.length)}</span></header><div>${items.map(item=>`<article class="pipeline-card" data-pipeline-id="${esc(item.id)}"><span class="type-badge ${item.format}">${item.format==="short"?"شورتز":"بلند"}</span><h3>${esc(shortTitle(item.title,62))}</h3><small>امتیاز اولیه ${n(item.score||0)}</small><div class="pipeline-actions">${stageIndex?`<button data-pipeline-action="back" title="مرحله قبل">→</button>`:""}<button data-pipeline-action="next">${stageIndex===pipelineStages.length-1?"ثبت انتشار":"مرحله بعد"}</button><button data-pipeline-action="remove" title="حذف">×</button></div></article>`).join("")||`<div class="pipeline-empty">—</div>`}</div></section>`}).join("")}
function evaluationCheckpoints(){return availableAnalysisVideos().slice(0,4).map(v=>{const age=Math.max(0,(Date.now()-new Date(v.publishedAt||v.date).getTime())/36e5),completed=age>=672?3:age>=168?2:age>=24?1:0,next=completed===0?`${n(Math.ceil(24-age))} ساعت تا ارزیابی اول`:completed===1?`${n(Math.ceil((168-age)/24))} روز تا ارزیابی ۷روزه`:completed===2?`${n(Math.ceil((672-age)/24))} روز تا جمع‌بندی ۲۸روزه`:"چرخهٔ ارزیابی کامل شد";return{v,completed,next}})}
function renderCheckpoints(){const root=document.getElementById("checkpointList");if(!root)return;root.innerHTML=evaluationCheckpoints().map(({v,completed,next})=>`<article class="checkpoint-card"><div><strong>${esc(shortTitle(v.title,58))}</strong><small>${esc(next)}</small></div><div class="checkpoint-track">${["۲۴ساعت","۷روز","۲۸روز"].map((x,i)=>`<span class="${i<completed?"done":i===completed?"current":""}">${i<completed?"✓ ":""}${x}</span>`).join("")}</div></article>`).join("")||`<div class="comment-empty">هنوز انتشاری برای ارزیابی نیست.</div>`}
function rescueAlerts(){const videos=availableAnalysisVideos().slice(0,12),ret=median(videos.map(v=>v.retention||0)),vel=median(videos.map(snapshotVelocity)),alerts=[];videos.filter(v=>Date.now()-new Date(v.publishedAt||v.date).getTime()<45*864e5).forEach(v=>{if(v.retention&&v.retention<ret*.82)alerts.push({id:v.id,type:"نگهداشت پایین",title:v.title,action:"هوک و ۳۰ ثانیهٔ اول را بازبینی کن",level:"high"});else if(snapshotVelocity(v)<vel*.55)alerts.push({id:v.id,type:"سرعت رشد پایین",title:v.title,action:"عنوان و تامنیل جایگزین را آزمایش کن",level:"medium"})});return alerts.slice(0,4)}
function renderRescue(){const root=document.getElementById("rescueList");if(!root)return;const alerts=rescueAlerts();document.getElementById("rescueCount").textContent=n(alerts.length);root.innerHTML=alerts.map(a=>`<article class="rescue-item ${a.level}"><span>!</span><div><b>${esc(a.type)}</b><strong>${esc(shortTitle(a.title,58))}</strong><small>${esc(a.action)}</small></div><button data-rescue-experiment="${esc(a.id)}">ساخت آزمایش</button></article>`).join("")||`<div class="all-good"><span>✓</span><strong>فعلاً هشدار فوری نداریم</strong><small>مربی پس از snapshot بعدی دوباره بررسی می‌کند.</small></div>`}
function experimentStatusLabel(status){return({planned:"طراحی",running:"در حال اجرا",completed:"تمام‌شده"})[status]||"طراحی"}
function renderExperiments(){const root=document.getElementById("experimentList");if(!root)return;root.innerHTML=state.experiments.slice().reverse().map(x=>`<article class="experiment-card" data-experiment-id="${esc(x.id)}"><div><span class="experiment-status ${x.status}">${experimentStatusLabel(x.status)}</span><strong>${esc(shortTitle(x.video,62))}</strong></div><small>A: ${esc(shortTitle(x.a,55))}</small><small>B: ${esc(shortTitle(x.b,55))}</small>${x.thumb?`<p>${esc(x.thumb)}</p>`:""}<div><button data-experiment-action="advance">${x.status==="completed"?"ثبت‌شده":"مرحله بعد"}</button><button data-experiment-action="remove">حذف</button></div></article>`).join("")||`<div class="comment-empty">هنوز آزمایشی ثبت نشده است.</div>`}
function renderWeeklyBrief(){const root=document.getElementById("weeklyBrief");if(!root)return;const current=sum(state.daily,"views"),previous=sum(state.comparisonDaily,"views"),change=percentChange(current,previous),idea=studioIdeas()[0],alerts=rescueAlerts(),schedule=nextPublishingSchedule(),goal=growthGoals[state.growthGoal];root.innerHTML=`<div class="brief-verdict ${change>=0?"positive":"warning"}"><span>${change>=0?"↗":"↘"}</span><div><strong>${change>=0?"این دوره حرکت رو به جلو است":"این دوره نیاز به یک اصلاح مشخص دارد"}</strong><small>${state.comparisonMode==="none"?"مقایسه خاموش است":`${n(Math.abs(change),"decimal")}٪ ${change>=0?"رشد":"افت"} بازدید نسبت به ${comparisonText()}`}</small></div></div><ol><li><b>بساز:</b> ${esc(idea?.title||"ایدهٔ اولویت‌دار پس از اتصال مشخص می‌شود")}</li><li><b>منتشر کن:</b> ${schedule.map(x=>`${x.format==="long"?"لانگ":"شورتز"} ${x.next.toLocaleDateString("fa-IR",{timeZone:"Asia/Tehran",weekday:"long"})} ساعت ${x.next.toLocaleTimeString("fa-IR",{timeZone:"Asia/Tehran",hour:"2-digit",minute:"2-digit"})}`).join("؛ ")}</li><li><b>اصلاح کن:</b> ${alerts[0]?esc(alerts[0].action):"ریتم فعلی را حفظ کن؛ هشدار فوری وجود ندارد"}</li></ol><div class="brief-goal"><span>هدف فعال: ${goal.label}</span><small>${goal.hint}</small></div>`}
function renderStudio(){if(!document.getElementById("view-studio"))return;document.querySelectorAll("[data-growth-goal]").forEach(b=>b.classList.toggle("active",b.dataset.growthGoal===state.growthGoal));renderIdeaQueue();renderPipeline();renderCheckpoints();renderRescue();renderExperiments();renderWeeklyBrief()}

function switchView(name){document.querySelectorAll(".view").forEach(x=>x.classList.remove("active"));document.querySelectorAll(".nav-item[data-view]").forEach(x=>x.classList.toggle("active",x.dataset.view===name));document.getElementById(`view-${name}`)?.classList.add("active");const titles={coach:"امروز قدم بعدی کانال را روشن کنیم.",studio:"ایده را به نتیجه و نتیجه را به یادگیری تبدیل کنیم.",overview:"این‌جا همهٔ عددها و شواهد پشت تصمیم‌هاست.",videos:"هر ویدئو، یک سرنخ برای تصمیم بعدی.",series:"ستون‌های محتوایی را با هم مقایسه کن.",audience:"ببین چه کسانی می‌آیند و چرا برمی‌گردند.",revenue:"درآمد را به عدد قابل‌استفاده تبدیل کن.",advisor:"پیشنهادهای اختصاصی برای قدم بعدی کانال.",comments:"گفت‌وگو با مخاطب، در یک صندوق واحد.",reports:"عددها را به برنامهٔ عملی تبدیل کن."};document.getElementById("pageTitle").textContent=titles[name];document.getElementById("pageSubtitle").textContent=name==="coach"?"برنامهٔ ساده، ارزیابی واقعی و مسیر قابل‌اصلاح":name==="studio"?"صف ایده، خط تولید، آزمایش و ارزیابی در یک چرخه":`عملکرد ${rangeText()} · مقایسه با ${comparisonText()}`;document.getElementById("sidebar").classList.remove("open");window.scrollTo({top:0,behavior:"smooth"})}
...
function classifyComment(text=""){if(/تبلیغ|واتساپ|تلگرام|درآمد تضمینی|http|www\./i.test(text))return{key:"spam",label:"مشکوک"};if(/[؟?]|چرا|چطور|کِی|کی |کجا|آیا/.test(text))return{key:"question",label:"پرسش"};if(/لطفاً|لطفا|بساز|موضوع|قسمت بعد|صحبت کنید|بررسی کنید/.test(text))return{key:"idea",label:"پیشنهاد موضوع"};if(/عالی|خوب|ممنون|سپاس|دوست داشتم/.test(text))return{key:"positive",label:"بازخورد مثبت"};if(/بد|ضعیف|اشتباه|غلط|افتضاح/.test(text))return{key:"critical",label:"انتقاد"};return{key:"general",label:"عمومی"}}
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  document.getElementById("growthGoalSwitcher")?.addEventListener("click",e=>{const goal=e.target.closest("[data-growth-goal]")?.dataset.growthGoal;if(!goal)return;state.growthGoal=goal;saveCoachState();renderStudio();showToast(`هدف مربی روی «${growthGoals[goal].label}» تنظیم شد`)});
  document.getElementById("ideaQueue")?.addEventListener("click",e=>{if(!e.target.closest('[data-idea-action="pipeline"]'))return;const id=e.target.closest(".idea-card")?.dataset.ideaId,idea=studioIdeas().find(x=>x.id===id);if(idea)addIdeaToPipeline(idea)});
  document.getElementById("contentPipeline")?.addEventListener("click",e=>{const action=e.target.closest("[data-pipeline-action]")?.dataset.pipelineAction,id=e.target.closest(".pipeline-card")?.dataset.pipelineId,item=state.pipeline.find(x=>x.id===id);if(!action||!item)return;const index=pipelineStages.findIndex(x=>x.key===item.stage);if(action==="next"){if(index===pipelineStages.length-1){item.stage="published";item.publishedAt=new Date().toISOString();showToast("انتشار ثبت شد؛ چرخهٔ ارزیابی آغاز می‌شود")}else item.stage=pipelineStages[index+1].key}if(action==="back"&&index>0)item.stage=pipelineStages[index-1].key;if(action==="remove"&&confirm("این کارت از خط تولید حذف شود؟"))state.pipeline=state.pipeline.filter(x=>x.id!==id);saveCoachState();renderStudio()});
  document.getElementById("addManualIdeaBtn")?.addEventListener("click",()=>document.getElementById("ideaDialog").showModal());document.getElementById("ideaForm")?.addEventListener("submit",e=>{e.preventDefault();const title=document.getElementById("manualIdeaTitle").value.trim(),format=document.getElementById("manualIdeaFormat").value,note=document.getElementById("manualIdeaNote").value.trim();if(!title)return;state.manualIdeas.unshift({id:uid("manual"),title,format,note,score:78,createdAt:new Date().toISOString()});saveCoachState();e.target.reset();document.getElementById("ideaDialog").close();renderStudio();showToast("ایده به صف اضافه شد")});
  ...
  document.getElementById("addExperimentBtn")?.addEventListener("click",()=>document.getElementById("experimentDialog").showModal());document.getElementById("experimentForm")?.addEventListener("submit",e=>{e.preventDefault();state.experiments.push({id:uid("experiment"),video:document.getElementById("experimentVideo").value.trim(),a:document.getElementById("experimentA").value.trim(),b:document.getElementById("experimentB").value.trim(),thumb:document.getElementById("experimentThumb").value.trim(),status:"planned",createdAt:new Date().toISOString()});saveCoachState();e.target.reset();document.getElementById("experimentDialog").close();renderStudio();showToast("آزمایش ثبت شد")});
  document.getElementById("experimentList")?.addEventListener("click",e=>{const action=e.target.closest("[data-experiment-action]")?.dataset.experimentAction,id=e.target.closest(".experiment-card")?.dataset.experimentId,x=state.experiments.find(y=>y.id===id);if(!action||!x)return;if(action==="advance")x.status=x.status==="planned"?"running":x.status==="running"?"completed":"completed";if(action==="remove")state.experiments=state.experiments.filter(y=>y.id!==id);saveCoachState();renderStudio()});
  document.getElementById("rescueList")?.addEventListener("click",e=>{const id=e.target.closest("[data-rescue-experiment]")?.dataset.rescueExperiment,v=availableAnalysisVideos().find(x=>x.id===id);if(!v)return;document.getElementById("experimentVideo").value=v.title;document.getElementById("experimentA").value=v.title;document.getElementById("experimentB").value="";document.getElementById("experimentDialog").showModal();document.getElementById("experimentB").focus()});
  ...
  document.getElementById("commentList")?.addEventListener("click",async e=>{const action=e.target.closest("[data-comment-action]")?.dataset.commentAction,id=e.target.closest(".comment-card")?.dataset.commentId;if(!action||!id)return;const comment=state.comments.find(c=>c.id===id);try{if(action==="reply")openReply(comment);if(action==="idea"){state.manualIdeas.unshift({id:uid("comment"),title:comment.text,format:"long",note:`از کامنت ${comment.author} زیر «${comment.videoTitle}»`,score:82,createdAt:new Date().toISOString()});saveCoachState();renderStudio();showToast("کامنت به صف ایده‌ها اضافه شد")}if(action==="approve")await moderateComment(id,"published");if(action==="reject"&&confirm("این کامنت از نمایش عمومی مخفی شود؟"))await moderateComment(id,"rejected")}catch(err){console.error(err);showToast("انجام این عملیات ممکن نشد")}});
